...
export default function OnboardingScreen({ navigation }: Props) {
  ...
  const fields: { key: keyof FormErrors; label: string; value: string; set: (v: string) => void; placeholder: string; keyboard?: KeyboardTypeOptions; transform?: (v: string) => string }[] = [
    { key: 'name', label: 'Nome', value: name, set: setName, placeholder: 'Seu nome' },
    { key: 'email' as any, label: 'E-mail', value: email, set: setEmail, placeholder: 'seu@email.com', keyboard: 'email-address' as KeyboardTypeOptions },
    { key: 'weight', label: 'Peso (kg)', value: weight, set: setWeight, placeholder: 'Ex: 70', keyboard: 'numeric' },
    { key: 'height', label: 'Altura (cm)', value: height, set: setHeight, placeholder: 'Ex: 175', keyboard: 'numeric' },
    { key: 'dob', label: 'Data de Nascimento', value: dob, set: (v) => setDob(maskDate(v)), placeholder: 'DD/MM/AAAA', keyboard: 'numeric' },
  ];
      ...
      {fields.map(({ key, label, value, set, placeholder, keyboard }) => (
        <View key={key} style={s.fieldGroup}>
          ...
          <TextInput
            ...
            value={value}
            onChangeText={set}
            onBlur={() => { touch(key); setErrors(validateAll(name, weight, height, dob, email)); }}
      ...
      <View style={s.fieldGroup}>
        ...
        <View style={s.sexRow}>
          {(['male', 'female'] as const).map(s_ => (
            <TouchableOpacity key={s_} style={[s.sexBtn, sex === s_ && s.sexBtnActive]} onPress={() => setSex(s_)}>
              <Text style={[s.sexText, sex === s_ && s.sexTextActive]}>{s_ === 'male' ? '♂ Masculino' : '♀ Feminino'}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
...
const s = StyleSheet.create({
  ...
  input: { backgroundColor: colors.card, color: colors.white, borderRadius: 12, padding: 14, fontSize: font.md, borderWidth: 1, borderColor: colors.border },
  inputError: { borderColor: '#EF4444' },
  errorText: { color: '#EF4444', fontSize: 12, marginTop: 4 },
  sexRow: { flexDirection: 'row', gap: 10 },
  sexBtn: { flex: 1, backgroundColor: colors.card, borderRadius: 12, padding: 14, alignItems: 'center', borderWidth: 1, borderColor: colors.border },
  ...
  btn: { backgroundColor: colors.green, borderRadius: 12, padding: 16, alignItems: 'center', marginTop: 8 },
