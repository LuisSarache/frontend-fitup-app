...
export function getBMICategory(bmi: number): BMICategory {
  if (bmi < 18.5) return { label: 'Abaixo do peso', color: '#60A5FA', description: 'Considere aumentar a ingestão calórica' };
  if (bmi < 25.0) return { label: 'Peso normal', color: '#22C55E', description: 'Continue assim! Seu peso está saudável' };
  if (bmi < 30.0) return { label: 'Sobrepeso', color: '#FBBF24', description: 'Exercícios regulares vão te ajudar' };
